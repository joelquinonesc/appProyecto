"""
ARCHIVADO: Versión combinada antigua del cuestionario SF-12

Este archivo fue movido a la carpeta `src/pages/archived/` el 2025-11-11
por razones de mantenimiento: la versión del SF-12 fue partida en dos
páginas (`sf12_fisica.py` y `sf12_mental.py`). Se conserva aquí para
referencia histórica y preservación del código original.
"""
...
def mostrar_sf12():
    ...
    with col2:
        if st.button("Siguiente →", key="btn_sf12_next", type="primary", disabled=disabled, width='stretch'):
            resultados = calcular_sf12(respuestas)
            # resultados es un dict con 'fisica', 'mental' y 'total'
            fisica = resultados.get('fisica')
            mental = resultados.get('mental')
            total = resultados.get('total')
            mensaje = "mejor" if (total is not None and total > 50) else "menor"
            ...
            with col1:
                st.metric(label="Puntaje físico (PCS)", value=f"{fisica:.1f}")
                st.metric(label="Puntaje mental (MCS)", value=f"{mental:.1f}")
            with col2:
            ...
            # Actualizar DataFrame dinámico con puntaje físico, su cuartil y etiqueta
...
def _mostrar_seccion_problemas(respuestas):
    ...
    st.markdown("<p style='color: #2E2E2E; font-size: 1.5rem; font-weight: 500; margin-bottom: 0.75rem; margin-top: 1rem;'><span style='color: #4CAF50; font-weight: 700;'>6.</span> ¿Hizo menos de lo que hubiera querido hacer, por algún problema emocional?</p>", unsafe_allow_html=True)
    resp = st.radio(
        "Pregunta 6",
        ["Sí", "No"],
        key="sf12_emo_1",
