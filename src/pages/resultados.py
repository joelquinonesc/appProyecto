...
def mostrar_resultados():
    ...
    if registro:
        ...
        # Use a single universal model for all cases; configurable via MODEL_PATH env var
        MODEL_PATH = os.environ.get('MODEL_PATH', 'src/models/mlp_no_gender_model_tuned.joblib')
        model_path = MODEL_PATH
        model_name = os.environ.get('MODEL_NAME', 'MLP (No Gender)')
        ...
        try:
            ...
            # ORDEN EXACTO DEL MODELO: EDAD24, AEFGROUPS, LTE12, SF12F, SF12M, PRKCA, TCF4, CDH20
            features_dict = {
                'EDAD24': edad24,
                'AEFGROUPS': aefgroups,
                'LTE12_0': lte12_0,
...
def mostrar_shap_analysis(model, X, genero):
    ...
    try:
        ...
        # Gráfico SHAP
        st.markdown("#### � Top 15 Características más Influyentes")
        top_n = 15
            ...
            if prob_alto is not None:
                ...
                try:
                    ...
                    if model is not None and X_for_model is not None:
                        ...
                        try:
                            if isinstance(model, MLPClassifier):
                                background_data = np.random.choice([0, 1], size=(50, X_array.shape[1]), p=[0.7, 0.3])
                                explainer = shap.KernelExplainer(model.predict_proba, background_data, feature_names=feature_names)
