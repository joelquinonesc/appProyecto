...
def mostrar_analisis_masivo():
    ...
    - **prkca**: *(opcional)* Genotipo (T/T, C/T, C/C). Dejar vacío si no disponible
    - **tcf4**: *(opcional)* Genotipo (A/A, A/T, T/T). Dejar vacío si no disponible
    - **cdh20**: *(opcional)* Genotipo (A/A, A/G, G/G). Dejar vacío si no disponible
    
    > 💡 Si las columnas genéticas están vacías o contienen "N/A", se usará el **modelo estándar (13 features)**.
    > Si tienen valores válidos, se usará el **modelo extendido (22 features)**.
    """)
    ...
    if uploaded_file is not None:
        try:
            ...
            # Validar columnas requeridas (genéticas son opcionales)
            ...
            if st.button("🔄 Procesar y Generar Reportes", key="process_button", use_container_width=True):
                ...
                # Tabla de transformaciones one-hot DETALLADA
                st.markdown("---")
                st.markdown("### 🔄 Codificación de Features para el Modelo CatBoost")
                
                # Crear tabla expandida con features usando funciones de calculos.py
                features_detallados = []
                for _, row in df.iterrows():
                    # Detectar si este paciente tiene datos genéticos válidos
                    tiene_gen = _paciente_tiene_genetica(row)
                    
                    # 1-2. TRANSFORMAR EDAD Y EDUCACIÓN usando funciones canónicas
                    edad24 = transformar_edad_a_grupo(int(row['edad']))
                    aefgroups = transformar_educacion_a_binaria(int(row['años_educacion']))
                    
                    # 3-5. LTE12 ONE-HOT usando función canónica
                    ...
                    # 6-9. SF-12 FÍSICA ONE-HOT de cuartiles
                    ...
                    # 10-13. SF-12 MENTAL ONE-HOT de cuartiles
...
def calcular_riesgo_paciente(row):
    ...
    try:
        ...
        # 1-2. TRANSFORMACIONES BINARIAS (funciones canónicas de calculos.py)
        edad24 = transformar_edad_a_grupo(int(row['edad']))
        aefgroups = transformar_educacion_a_binaria(int(row['años_educacion']))
        
        # 3-5. LTE12 ONE-HOT (función canónica)
        lte12_clasif = transformar_lte12_a_clasificacion(int(row['lte12_count']))
        lte12_0 = 1 if lte12_clasif == 0 else 0
        lte12_1 = 1 if lte12_clasif == 1 else 0
        lte12_2 = 1 if lte12_clasif == 2 else 0
        
        # 6-9. SF-12 FÍSICA ONE-HOT
        sf12f_cuartil = transformar_sf12_fisica_a_cuartil(float(row['sf12_fisica']))
        sf12f_q1 = 1 if sf12f_cuartil == 1 else 0
        sf12f_q2 = 1 if sf12f_cuartil == 2 else 0
        sf12f_q3 = 1 if sf12f_cuartil == 3 else 0
        ...
        # Usar el modelo CatBoost correspondiente
        try:
        ...
        # Determinar categoría de riesgo con umbrales canónicos
...
def calcular_riesgo_simple(row):
    ...
    """
    # Normalizar scores
    hads_norm = min(float(row['hads_score']) / 42, 1.0)  # 0-42
    zsas_norm = min((float(row['zsas_score']) - 20) / 60, 1.0)  # 20-80
    sf12_mental_norm = 1 - (float(row['sf12_mental']) / 100)  # Invertido: menos salud = más riesgo
